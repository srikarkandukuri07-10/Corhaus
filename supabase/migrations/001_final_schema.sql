-- CORHAUS PILATES PLATFORM - Complete Schema & Fix (Idempotent)
...
DROP POLICY IF EXISTS "Members can insert own bookings" ON bookings;
...
-- Enable realtime (ignore if already member)
DO $$
BEGIN
  IF NOT EXISTS (
