...
// Check if table exists; if not, create it via exec_sql RPC
async function ensureStaffTable(serviceClient: any): Promise<boolean> {
  try {
    const { error } = await serviceClient.from("staff_members").select("id").limit(1);
    ...
    if (isMissing) {
      console.warn("staff_members table missing — attempting auto-create via exec_sql RPC");
      const { error: rpcErr } = await serviceClient.rpc("exec_sql", { sql: CREATE_TABLE_SQL });
      if (rpcErr) {
    ...
    console.error("ensureStaffTable error:", e);
    return false;
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    await ensureStaffTable(serviceClient);
    ...
    const sortBy = searchParams.get("sortBy") || "name"; // 'name', 'joinDate', 'role'
    ...
    if (error) {
      // If table is still missing after ensure attempt, return empty gracefully
      const isMissing =
        error.message?.includes("does not exist") ||
        error.message?.includes("schema cache") ||
        error.code === "42P01" ||
      ...
      if (isMissing) {
        return NextResponse.json({
          staff: [],
          summary: { totalStaff: 0, totalTrainers: 0, activeStaff: 0, monthlyPayroll: 0 },
          _tableNotReady: true,
        });
      }
