...
async function ensureStaffTable(serviceClient: any) {
  try {
    // Probe table
    const { error } = await serviceClient.from("staff_members").select("id").limit(1);
    if (error && (error.message?.includes("does not exist") || error.code === "42P01")) {
      // Execute table creation query if RPC exec available or return schema instructions
      console.warn("staff_members table probed but missing in public schema.");
    }
  } catch (e) {
