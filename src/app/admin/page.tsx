  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/admin/classes"
          className="px-5 py-2.5 rounded-xl bg-accent text-white text-sm font-semibold hover:bg-accent-2 transition-colors shadow-md flex items-center gap-1.5"
        >
          <span>+</span> New Class
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Today's Classes */}
        <div className="bg-surface rounded-[18px] p-6 border border-line shadow-xs flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            <p className="text-xs font-bold text-fg-3 tracking-wider uppercase">
              Today&apos;s Classes
            </p>
            <p className="text-3xl font-extrabold text-fg mt-2">
              {loading ? "..." : todaysClassesCount}
            </p>
            <Link
              href="/admin/classes"
              className="inline-flex items-center gap-1 text-xs font-bold text-accent hover:underline mt-3"
            >
        ...
        <div className="bg-surface rounded-[18px] p-6 border border-line shadow-xs flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <Link
              href="/admin/members"
              className="inline-flex items-center gap-1 text-xs font-bold text-blue-500 hover:underline mt-3"
            >
        ...
        <div className="bg-surface rounded-[18px] p-6 border border-line shadow-xs flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <Link
              ...
              className="inline-flex items-center gap-1 text-xs font-bold text-gold hover:underline mt-3"
            >
              View details &rarr;
            </Link>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-gold/10 text-gold flex items-center justify-center flex-shrink-0 font-bold text-xl">
            ₹
          </div>
        </div>
        ...
        <div className="bg-surface rounded-[18px] p-6 border border-line shadow-xs flex items-center justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <Link
              href="/admin/scanner"
              className="inline-flex items-center gap-1 text-xs font-bold text-green-500 hover:underline mt-3"
            >
              View scanner &rarr;
            </Link>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-green-500/10 text-green-500 flex items-center justify-center flex-shrink-0">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-fg">Today&apos;s Classes</h2>
          <Link
            href="/admin/classes"
        ...
        {loading ? (
          ...
          <div className="bg-surface rounded-[22px] border border-line p-12 text-center shadow-xs space-y-3">
            ...
            <p className="text-sm font-semibold text-fg">No classes scheduled for today</p>
            <Link
              href="/admin/classes"
            ...
            {classes.map((c) => {
              ...
              return (
                <div
                  ...
                  className={`bg-surface rounded-[18px] border p-5 cursor-pointer transition-all ${
                    selectedClass === c.id
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="font-bold text-base text-fg">{c.title}</h3>
                      <p className="text-xs text-fg-3 mt-0.5">{c.instructor}</p>
                    </div>
                    <span
                  ...
                  <div className="mt-4 pt-3 border-t border-line flex items-center justify-between text-xs text-fg">
                    <span>
      ...
      {selectedClass && selectedClassData && (
        <div className="bg-surface rounded-[18px] border border-line p-6 shadow-md space-y-4 animate-slide-up">
          <div className="flex items-center justify-between border-b border-line pb-4">
            ...
            <button
              ...
              className="text-xs font-bold text-fg-3 hover:text-fg"
            >
              ✕ Close
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-fg-3 uppercase tracking-wider">
                Booked Members ({bookings.length})
              </h4>
              {bookingsLoading ? (
                <div className="py-6 text-center text-xs text-fg-4">Loading bookings...</div>
              ) : bookings.length === 0 ? (
                <div className="py-6 text-center text-xs text-fg-4 bg-surface-2 rounded-xl">
            ...
            <div className="space-y-3">
              ...
              {attendanceLoading ? (
                <div className="py-6 text-center text-xs text-fg-4">Loading attendance...</div>
              ) : attended.length === 0 ? (
                <div className="py-6 text-center text-xs text-fg-4 bg-surface-2 rounded-xl">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                  {attended.map((a) => (
                    <div
                      key={a.id}
                      className="p-3 rounded-xl bg-green-500/10 border border-green-500/20 flex items-center justify-between text-xs text-green-700"
                    >
                      <div>
                        <p className="font-bold">{a.profiles?.full_name || "Member"}</p>
                        <p className="text-[11px] text-green-600">{a.profiles?.email}</p>
                      </div>
                      <span className="px-2 py-0.5 rounded-full bg-green-500/20 text-green-700 font-bold text-[10px]">
                        ATTENDED
                      </span>
                    </div>
                  ))}
