...
export default function TrialMembersPage() {
  ...
  // Fetch Leads & Enquiries and dropdown master data
  const fetchTrialMembers = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Failed to fetch Leads & Enquiries");
      }
      const json = await res.json();
      setTrialMembers(json.data || []);
    } catch (err: any) {
      console.error(err);
      setError(err.message || "Failed to load Leads & Enquiries");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleUpdateStatus = async (trialId: string, status: "Attended" | "No Show" | "Scheduled") => {
    try {
      const res = await fetch(`/api/admin/leads/${trialId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleReschedule = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/leads/${reschedulingTrial.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/leads/${editingTrial.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-line-2 pb-5">
        <div>
          ...
          <h1 className="text-2xl font-serif font-bold text-fg">Leads & Enquiries Management</h1>
      ...
      {/* Leads & Enquiries Dashboard Data Table */}
      <div className="bg-surface rounded-2xl border border-line-2 shadow-xs overflow-hidden">
        {loading ? (
          <div className="p-12 flex flex-col items-center justify-center text-fg-3">
            ...
            <p className="text-xs font-semibold">Loading Leads & Enquiries...</p>
          </div>
          ...
          <div className="p-12 text-center text-fg-3">
            ...
            <p className="text-sm font-bold text-fg-2">No Leads & Enquiries found</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-surface border border-line-2 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-line-2 pb-3">
              <h3 className="text-lg font-serif font-bold text-fg">+ Create Lead</h3>
              <button
      ...
      {editingTrial && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-surface border border-line-2 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-line-2 pb-3">
              <h3 className="text-lg font-serif font-bold text-fg">Edit Lead</h3>
              <button
