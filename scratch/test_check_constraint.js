...
async function testCheckConstraint() {
  ...
  const id = bks[0].id;
  const statuses = ['booked', 'confirmed', 'checked_in', 'completed', 'attended', 'cancelled', 'no_show', 'waitlisted'];
  
  for (const s of statuses) {
    const res = await supabase.from('bookings').update({ booking_status: s }).eq('id', id);
    console.log(`Status '${s}':`, res.error ? res.error.message : 'ALLOWED!');
  }
}
